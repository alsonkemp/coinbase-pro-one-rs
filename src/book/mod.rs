...
use futures_util::{StreamExt, stream::FilterMap};
use uuid::Uuid;
...
impl OrderBook {
    ...
    pub async fn harvest<'a>(&mut self, strm: Pin<Box<Receiver<Message>>>) -> BoxStream<'a, Message>
    {
        strm.filter_map(|msg| async move {
            match msg {
                structs::Message::Level2(Level2::Snapshot{
                    product_id,
                    bids,
                    asks}) => {
                        debug!("BOOK: {:?} {:?} {:?}", product_id, bids, asks);
                        None },
                structs::Message::Level2(Level2::Snapshot{
                                             product_id,
                                             bids,
